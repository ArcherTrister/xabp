...
namespace X.Abp.CmsKit.Polls
{
    public class MongoPollRepository : MongoDbRepository<ICmsKitProMongoDbContext, Poll, Guid>, IPollRepository
    {
          ...
          CancellationToken cancellationToken = default)
        {
            ...
            return await (await GetMongoQueryableAsync(cancellationToken, null)).FirstOrDefaultAsync(poll => poll.Widget == widget && ((poll.StartDate <= now && (poll.EndDate.HasValue == false || poll.EndDate >= (DateTime?)now)) || (poll.ResultShowingEndDate.HasValue && poll.ResultShowingEndDate >= (DateTime?)now)), GetCancellationToken(cancellationToken));
        }
          ...
          Guid userId)
        {
            ...
            return await (await GetMongoQueryableAsync(new CancellationToken()))
                .GroupJoin(dbContext.PollUserVotes, (Expression<Func<Poll, Guid>>)(poll => poll.Id), pollUserVote => pollUserVote.PollId, (poll, pollUserVotes) => new
                {
                    poll = poll,
