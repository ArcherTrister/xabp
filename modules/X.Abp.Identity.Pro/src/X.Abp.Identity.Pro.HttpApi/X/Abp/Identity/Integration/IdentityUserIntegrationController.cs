...
namespace X.Abp.Identity.Integration
{
      ...
      IIdentityUserIntegrationService
    {
        ...
        [HttpGet]
        [Route("{id}")]
        public Task<UserData> FindByIdAsync(Guid id) => UserIntegrationService.FindByIdAsync(id);

        [Route("by-username/{userName}")]
        [HttpGet]
        public Task<UserData> FindByUserNameAsync(string userName) => UserIntegrationService.FindByUserNameAsync(userName);

        [Route("search")]
        [HttpGet]
        public Task<ListResultDto<UserData>> SearchAsync(
        ...
        [HttpGet]
        [Route("count")]
        public Task<long> GetCountAsync(UserLookupCountInputDto input) => UserIntegrationService.GetCountAsync(input);
    }
}
