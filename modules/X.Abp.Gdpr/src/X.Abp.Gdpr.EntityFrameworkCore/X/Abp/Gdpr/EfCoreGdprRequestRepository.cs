...
IGdprRequestRepository
{
  ...
  public virtual async Task<long> GetCountByUserIdAsync(
    Guid userId,
  ...
  public virtual async Task<List<GdprRequest>> GetListByUserIdAsync(
    Guid userId,
    CancellationToken cancellationToken = default)
  {
  ...
  public virtual async Task<DateTime?> FindLatestRequestTimeOfUserAsync(
    Guid userId,
    CancellationToken cancellationToken = default)
  {
    return await (await GetQueryableAsync()).Where(gdprRequest => gdprRequest.UserId == userId).OrderByDescending(gdprRequest => gdprRequest.CreationTime).Select(gdprRequest => gdprRequest.CreationTime).FirstOrDefaultAsync(GetCancellationToken(cancellationToken));
  }
